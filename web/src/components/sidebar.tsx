...
import { Search, Clock, Users, Share2, Settings, Brain, Plus, X } from "lucide-react";
...
export function Sidebar({
  ...
  return (
    <aside className="w-60 h-dvh bg-zinc-950 border-r border-zinc-800 flex flex-col">
      <div className="p-4 flex items-center gap-3">
        <Brain className="h-6 w-6 text-blue-500 shrink-0" />
        <span className="font-semibold text-sm">Open Brain</span>
        {onNavigate && (
          <Button variant="ghost" size="icon-sm" onClick={onNavigate} className="ml-auto lg:hidden" aria-label="Close navigation">
            <X className="h-4 w-4" />
          </Button>
        )}
      </div>
      ...
      <div className="p-2 border-t border-zinc-800">
        <button onClick={() => { onQuickCapture(); onNavigate?.(); }}
